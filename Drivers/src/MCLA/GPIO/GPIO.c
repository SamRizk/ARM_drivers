...
GPIO_ERROR_STATE_t GPIO_toggle(void * port, GPIO_PINS_t pin)
{


    GPIO_ERROR_STATE_t error_state = GPIO_OK;
    u32 tmpBuffer,pinnumber;
    if(IS_PORT(port) == 0 ||IS_GPIO(pin) == 0)
    {
        error_state = GPIO_ERROR_INVALID_PARAM;
    }
    ...
    else
    {
            ...
            tmpBuffer = ((GPIO*)port)->GPIO_ODR;
            pin &= PIN_CLR_MASK;
            pinnumber= GetPinNumber(pin);
            tmpBuffer = TGL_BIT(tmpBuffer,pinnumber);
            ((GPIO*)port)->GPIO_ODR = tmpBuffer;
    }
    return error_state;
}

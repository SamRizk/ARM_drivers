...
NVIC_enuErrorStatus_t NVIC_SetPriority(IRQn_t IRQn, u32 priority)
{
    NVIC_enuErrorStatus_t LOC_status = NVIC_OK;
    if ((u32)(IRQn) < 0)
    {
...
NVIC_enuErrorStatus_t NVIC_getPriority(IRQn_t interruptnum ,u32* priority)
{
    ...
    {
        if ((u32)(interruptnum) < 0)
        {
          *priority= ((u32)SCB->SHP[(((u32)interruptnum) & 0xFUL)-4UL] >> ((NUMBER_OF_PRI_BITS - NVIC_PRIO_BITS)));
        }
        else
        {
          *priority= ((u32)NVIC->IP[((u32)interruptnum)] >> ((NUMBER_OF_PRI_BITS - NVIC_PRIO_BITS)));
        }
    }
